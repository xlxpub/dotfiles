...
### ~~🤖 claudecode.nvim~~（已移除）
`coder/claudecode.nvim` 已从本机配置删除，仅保留 `snacks.nvim`（终端功能）。
...
结果写入系统剪贴板（`+` 寄存器），可直接 Cmd+V 粘贴。函数名通过 treesitter 向上遍历语法树获取，支持 Go / Lua / JS / TS / Python。
...
### 🎨 主题切换为 GitHub 浅色风格（已被 unokai 取代）
- 插件：`projekt0n/github-nvim-theme`
- colorscheme：`github_light_default` → `github_dark_dimmed` → `unokai`
